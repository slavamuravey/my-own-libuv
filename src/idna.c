...
void uv_wtf8_to_utf16(const char* source_ptr, uint16_t* w_target, size_t w_target_len) {
  int32_t code_point;
...
static int32_t uv__get_surrogate_value(const uint16_t* w_source_ptr, ssize_t w_source_len) {
  uint16_t u;
  uint16_t next;
...
size_t uv_utf16_length_as_wtf8(const uint16_t* w_source_ptr, ssize_t w_source_len) {
  size_t target_len;
  int32_t code_point;
...
int uv_utf16_to_wtf8(const uint16_t* w_source_ptr, ssize_t w_source_len, char** target_ptr, size_t* target_len_ptr) {
  size_t target_len;
  char* target;
  char* target_end;
  int32_t code_point;
